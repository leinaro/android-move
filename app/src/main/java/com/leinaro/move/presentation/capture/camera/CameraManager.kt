...
import com.leinaro.move.presentation.capture.camera.AutoFocusManager
import com.leinaro.move.presentation.capture.camera.open.OpenCameraInterface
...
class CameraManager(private val context: Context) {
   ...
   */
  private val previewCallback: PreviewCallback = PreviewCallback(configManager)
   ...
   */
  @Synchronized @Throws(IOException::class) fun openDriver(holder: SurfaceHolder?) {
  ...
  @get:Synchronized val isOpen: Boolean
    get() = camera != null

  /**
   * Closes the camera driver if still in use.
   */
  @Synchronized fun closeDriver() {
    if (camera != null) {
   ...
   */
  @Synchronized fun setTorch(newSetting: Boolean) {
    val theCamera = camera
   ...
   */
  @Synchronized fun requestPreviewFrame(handler: Handler?, message: Int) {
    val theCamera = camera
    if (theCamera != null && previewing) {
   ...
   */
  @Synchronized fun getFramingRect(): Rect? {
    if (framingRect == null) {
      if (camera == null) {
        return null
      }
   ...
   */
  @Synchronized fun getFramingRectInPreview(): Rect? {
   ...
   */
  @Synchronized fun setManualFramingRect(width: Int, height: Int) {
    var width = width
    var height = height
    if (initialized) {
      val screenResolution = configManager.screenResolution
